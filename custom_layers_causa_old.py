    ...
    * `expert` can be specified as a module class, it is used to generate
    ...
    def forward(self, moe_inp,attn_weights):
        ...
        with torch.no_grad():
            ...
            num_subgraphs=4
            for add_index in range(64):
                rets = cal_expert(attn_weights[:,add_index*32:add_index*32+32,:], num_subgraphs, moe_inp.shape[-1])
                for i in range(len(rets)):
                    for j in range(len(rets[i])):
                        for k in range(1, len(rets[i][j])):
                            moe_inp[i][rets[i][j][k]+32*add_index] = moe_inp[i][rets[i][j][k - 1]+32*add_index]
